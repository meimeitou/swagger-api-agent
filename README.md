...
- **对话历史**: 支持上下文对话，记录调用历史
- **灵活配置**: 支持多种认证方式和自定义配置
- **🔐 多用户认证**: 支持用户登录认证，每个用户拥有独立的对话会话
- **🔄 会话管理**: 自动管理用户会话，支持会话隔离和自动清理
- **⏰ Token 管理**: 自动检测 Token 过期，支持多标签页同步登出
- **👥 多用户支持**: 支持多个用户同时使用，各自维护独立的 Agent 实例和对话历史
...
- **用户登录**: 基于 JWT Token 的安全认证
- **会话隔离**: 每个用户拥有独立的 Agent 实例和对话历史
- **自动过期**: Token 自动过期检测和清理
- **多标签页同步**: 支持多标签页登录状态同步
- **管理功能**: 提供用户会话管理和监控接口
...
- `GET /api/admin/sessions` - 查看所有会话（管理员）
